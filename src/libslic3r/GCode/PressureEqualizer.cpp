...
PressureEqualizer::PressureEqualizer(const Slic3r::GCodeConfig &config, bool spiral_vase_enabled) : m_use_relative_e_distances(config.use_relative_e_distances.value)
{
    ...
    if(config.max_volumetric_extrusion_rate_slope.value > 0){
        ...
        // Orca: In spiral vase mode there is no Z seam - the external perimeter is one continuous helical
        // extrusion with no start/end travel move - so seeding a flow ramp there would only needlessly
        // slow the print down.
        m_smooth_external_perimeter_seams = m_extrusion_rate_smoothing_external_perimeter_only && !spiral_vase_enabled;
    }
...
void PressureEqualizer::process_layer(const std::string &gcode)
{
    ...
    if (m_smooth_external_perimeter_seams)
