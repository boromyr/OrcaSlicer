...
public:
    ...
    // Orca: spiral_vase_enabled comes from PrintConfig::spiral_mode, which is not part of GCodeConfig.
    explicit PressureEqualizer(const Slic3r::GCodeConfig &config, bool spiral_vase_enabled = false);
...
private:
    ...
    // Orca: same as above, but also disabled in spiral vase mode, where there is no Z seam to ramp
    // the flow around. Guards the call to smooth_external_perimeter_seams().
    bool                           m_smooth_external_perimeter_seams = false;
