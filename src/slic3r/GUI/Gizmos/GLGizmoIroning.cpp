...
void GLGizmoIroning::on_shutdown()
{
    m_parent.use_slope(false);
    m_parent.toggle_model_objects_visibility(true);
}

std::string GLGizmoIroning::on_get_name() const
{
    return _u8L("Paint-on ironing");
}

bool GLGizmoIroning::on_init()
{
    // "I" is Zoom in and every ironing-adjacent letter is already a gizmo shortcut, so "J" it is.
    ...
    m_desc["add_ironing"]       = _L("Add ironing");
    m_desc["remove_ironing"]    = _L("Remove ironing");
    m_desc["smart_fill_angle"]  = _L("Smart fill angle");

    std::pair<wxString, wxString> add_ironing_shortcut    = {_L("Left mouse button"),         m_desc["add_ironing"]};
    std::pair<wxString, wxString> remove_ironing_shortcut = {shift + _L("Left mouse button"), m_desc["remove_ironing"]};
    std::pair<wxString, wxString> clipping_shortcut       = {alt + _L("Mouse wheel"),         m_desc["clipping_of_view"]};

    m_shortcuts_brush = {
        add_ironing_shortcut,
        remove_ironing_shortcut,
...
void GLGizmoIroning::render_tooltip_button(float x, float y)
{
...
void GLGizmoIroning::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    for (const std::string t : {"add_ironing", "remove_ironing"}) {
    ...
    ImGui::SameLine();
    m_imgui->disabled_begin(mo->is_ironing_painted() == false);
    ...
    const bool                has_object_ironing_override = obj_cfg.option("ironing_type");
    const IroningType         effective_ironing_type      = has_object_ironing_override ? obj_cfg.opt_enum<IroningType>("ironing_type")
                                                                                        : glb_cfg.opt_enum<IroningType>("ironing_type");
    // The painting only takes effect when the object actually irons the painted surfaces.
    if (effective_ironing_type != IroningType::PaintedOnly) {
        ...
        auto link_text = [&]() {
            ...
            m_imgui->text_wrapped(_L("Warning: Ironing type is not set to painted surfaces, the painting will not take effect!"), parent_width);
            ImGui::PopStyleColor();
            ImGui::PushStyleColor(ImGuiCol_Text, HyperColor.Value);
            ImGui::Dummy(ImVec2(font_size * 1.8f, font_size * 1.3f));
            ImGui::SameLine();
            m_imgui->bold_text(_u8L("Iron only the painted surfaces of this object"));
            ImGui::PopStyleColor();
...
void GLGizmoIroning::update_model_object()
{
    bool         updated = false;
    ModelObject *mo      = m_c->selection_info()->model_object();
    int          idx     = -1;
    for (ModelVolume *mv : mo->volumes) {
        ...
        ++idx;
        updated |= mv->ironing_facets.set(*m_triangle_selectors[idx]);
    }
...
void GLGizmoIroning::update_from_model_object(bool first_update)
{
    wxBusyCursor wait;
    ...
    for (const ModelVolume *mv : mo->volumes) {
        ...
        m_triangle_selectors.back()->deserialize(mv->ironing_facets.get_data(), false);
        m_triangle_selectors.back()->request_update_render_data();
    }
}

PainterGizmoType GLGizmoIroning::get_painter_type() const
{
    return PainterGizmoType::IRONING;
}

wxString GLGizmoIroning::handle_snapshot_action_name(bool shift_down, GLGizmoPainterBase::Button button_down) const
{
    return shift_down ? _L("Remove ironing") : _L("Add ironing");
}
