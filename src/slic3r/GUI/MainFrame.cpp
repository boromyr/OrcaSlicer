...
// Alt+Space opens the speed dial, but plain Space is the activation key for buttons, checkboxes
// and other controls. CHAR_HOOK runs before the focused child, so only take Space when the
// focused window has no keyboard-activation meaning of its own. Canvases (GLCanvas3D) and panels
// are not controls and fall through to "open"; the Notebook itself does too, so Alt+Space still
// opens the dial on any page.
...
;    }
    this->Bind(wxEVT_CHAR_HOOK, [this](wxKeyEvent &evt) {
        ...
        if (evt.CmdDown() && evt.RawControlDown() && evt.GetKeyCode() == 'F') {
            ...
            return;}
#endif
        // Orca: open the speed dial from any page with Alt+Space (not bare Space - that's the
        // canvas's own legend-fold toggle, and the two would otherwise fight over the same key).
        // The focused window still gets first refusal if it uses Space to activate itself
        // (buttons, checkboxes, list/choice controls, text fields), so Alt+Space there still
        // clicks/toggles instead of being hijacked. Gated by a preference (default on) so users
        // can hand Space back to the focused control entirely.
...
void MainFrame::init_menubar_as_editor()
{
    ...
    append_menu_item(
        parent_menu, wxID_ANY, _L("Open speed dial...") + sep + "Alt+Space", "",
    ...
    append_menu_item(
        top_menu, wxID_ANY, _L("Open speed dial...") + "\t" + "Alt+Space", "",
        [](wxCommandEvent &) { wxGetApp().open_speed_dial(); },
        "", nullptr, []() { return true; }, this);
    top_menu->AppendSeparator();
    ...
    fileMenu->AppendSeparator();
    append_menu_item(
        fileMenu, wxID_ANY, _L("Open speed dial...") + sep + "Alt+Space", "",
        [](wxCommandEvent&) { wxGetApp().open_speed_dial(); },
        "", nullptr, []() { return true; }, this);
    append_menu_item(
